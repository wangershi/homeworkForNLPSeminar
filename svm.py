'''
    follow https://blog.csdn.net/weixin_39605679/article/details/81170300
    support high-dimension vector, not only x-y data
'''
# -*-coding:utf-8 -*-
...
def loadDataSet(fileName, ):
    ...
    labelMat = np.mat(dataset[:, 0]).transpose()
...
def showClassifer(dataMat, classLabels, w, b):
    ...
    #绘制直线
    x1 = max(dataMat[:, 0])[0, 0]
    x2 = min(dataMat[:, 0])[0, 0]
    a1, a2 = w
    b = float(b)
    a1 = float(a1[0])
    a2 = float(a2[0])
...
if __name__ == '__main__':
    if False:
        dataArr, classLabels = loadDataSet('testSet.tsv')
